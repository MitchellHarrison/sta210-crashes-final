...
crashes %>%
  ...
  labs(x = "Vehicle Type", y = "Casualty Rate", fill = "Casualty", title = "Casualties Occur More Frequently in Accidents
with Motorcycles or Non-Motor Vehicles", subtitle = "MVCs with Casualties based on Vehicle Type 1")

# vtype2
crashes %>%
  filter(vtype2 != "None" & vtype2 != "Other/Unknown") %>%
  ggplot(aes(x = vtype2, fill = has_casualty)) +
  geom_bar(position = "fill", color = "black") + 
  scale_fill_manual(values = c("blue", "red"), labels = c("No", "Yes")) +
  labs(x = "Vehicle Type", y = "Casualty Rate", fill = "Casualty", title = "Casualties Occur More Frequently in Accidents
with Motorcycles or Non-Motor Vehicles", subtitle = "MVCs with Casualties based on Vehicle Type 2")
...
crashes %>%
  ...
  geom_bar(position = "fill", color = "black") + 
  scale_fill_manual(values = c("blue", "red"), labels = c("No", "Yes")) +
  labs(x = "Factor Type", y = "Casualty Rate", fill = "Casualty", title = "No Clearly More Dangerous Driver Factor", subtitle = "MVCs with Casualties based on Factor Type 1")

# factor2
crashes %>%
  filter(factor2 != "Other/Unknown") %>%
  ggplot(aes(x = factor2, fill = has_casualty)) +
  geom_bar(position = "fill", color = "black") + 
  scale_fill_manual(values = c("blue", "red"), labels = c("No", "Yes")) +
  labs(x = "Factor Type", y = "Casualty Rate", fill = "Casualty", title = "No Clearly More Dangerous Driver Factor", subtitle = "MVCs with Casualties based on Factor Type 2")
```
...
# weekend prop
(crashes %>%
  filter(weekend_weekday == "Weekend" & crash_time %in% c(00:00, 04:00) & factor1 == "Impairment/Distraction/Fatigue") %>%
  count())/(crashes %>%
  filter(weekend_weekday == "Weekend" & crash_time %in% c(00:00, 04:00)) %>%
  count())
...
**I dont think we need this plot below. To be honest, it shows that something like 300 streets have casualty rates around 0.5, which is pretty high and probably would indicate that we should include street somehow as a predictor...**
