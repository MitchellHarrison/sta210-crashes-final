...
A similar analysis (listed in appendix 2c) shows no significant difference in casualty rates among categories.
...
However, there are several limitations to this analysis that must be acknowledged. Firstly, the data used in this study was harvested and compiled from NYPD open access data, which means that the reliability and validity of the data may not be ideal. Furthermore, there is a possibility that some incidents of MVCs were not reported to the police, which could cause unaccounted missingness. It is also worth noting that the dataset is limited to New York City and may not be generalizable to other metropolitan areas. Future studies should attempt to replicate this analysis in other urban areas to determine if the findings are consistent across different locations.

Additionally, while the model provides insight into which characteristics of an MVC contribute to a higher likelihood of a casualty, it does not account for external factors that may have an impact on the likelihood of an MVC occurring in the first place. For example, weather conditions, road quality, and traffic volume could all contribute to the likelihood of an MVC occurring and should be considered in future analyses. The study could also benefit from including more detailed information about the drivers involved in the MVCs, such as age, gender, and driving experience, as these variables may also contribute to the likelihood of a casualty.
...
#### b) Odds Ratios and Probabilities from Log(odds) slope estimates

```{r}
#| label: Probs-from-model
finalmodel_tidy <- finalmodel_tidy %>%
  mutate(prob = exp(estimate)/(1+exp(estimate)),
         odds = exp(estimate))

finalmodel_tidy %>%
  dplyr::select(term, odds, prob, p.value)
