...
We constructed and compared 3 models using a likelihood ratio test to select our final model, each with `has_casualty` as the response variable. Predictors in model 1 (the base model) are `vtype1`, `vtype2`, `time_day`, `weekend_weekday`, and `yday`, with model 2 adding `factor1` and `factor2`, and model 3 adding `weekend_weekday*time_day` to model 2.

```{r}
#| label: likelihoodratio
# base model
model_base <- glm(has_casualty ~ vtype1 + vtype2 + time_day +
...
anova(model_base, model_2, model_3, test = "Chisq")
