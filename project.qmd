...
Based on our likelihood ratio test (output shown in **Appendix 3a**), `model_2` fits our data better than model 1, while the difference between `model_2` and `model_3` is insignificant. We move forward with `model_2` (since it has fewer predictors than `model_3`) and test assumptions of independence and linearity.
